...
function sendMessangeToTg(messageText, type, totalCost = 0) {
    createDialogBox('preloader', 'Отправка...');

    setTimeout(() => {
        const chatId = '7705038030';
        const serverUrl = "https://functions.yandexcloud.net/d4erht5n4er2utkuhfuc";
...
function saveUserData() {
    console.log("вот " + tableNumber);


    const userData = {
        basket: BASKET_DATA_STORE,
        order: ORDER_DATA_STORE,
        orderNumberHtml: orderNumberForHtml,
        orderNumberTg: orderNumberForTg,
...
function checkSaveUserData() {
    if (localStorage.getItem('userData')) {
        const savedUserData = JSON.parse(localStorage.getItem('userData'));

        if (savedUserData.basket) {
            BASKET_DATA_STORE = savedUserData.basket;
        }

        if (savedUserData.order && savedUserData.orderNumberHtml) {
            ORDER_DATA_STORE = savedUserData.order;
            orderNumberForHtml = savedUserData.orderNumberHtml;
        ...
        if (savedUserData.orderNumberTg) {
            orderNumberForTg = savedUserData.orderNumberTg;
        }
